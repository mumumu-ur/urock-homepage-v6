...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    _render() {
      ...
      // The host page has no server-side render step, so on the very first
      // attributeChangedCallback the `src` attribute can still be the raw,
      // un-substituted template text (e.g. "{{ q.img }}") rather than the
      // real URL — the host template engine hasn't resolved it into this
      // attribute yet. Treat that as "not set yet" instead of fetching the
      // literal placeholder text; the real value's own attributeChangedCallback
      // fires _render() again moments later once the engine resolves it.
